...
namespace TaleWorlds.TwoDimension
{
	...
	public class EditableText : RichText
	{
		...
		public EditableText(int width, int height, Font font, Func<int, Font> getUsableFontForCharacter) : base(width, height, font, getUsableFontForCharacter)
		{
			...
			string pattern = "\\w+";
			this._nextWordRegex = new Regex(pattern);
		}
		...
		public Vector2 GetCursorPosition(Font font, float fontSize, float scale)
		{
			...
			string word = this.VisibleText.Substring(this._visibleStart, this.CursorPosition - this._visibleStart);
		...
		private void UpdateSelectedText(Vector2 mousePosition)
		{
			...
			{
				this.SelectedTextBegin = this.FindCharacterPosition(this.VisibleText, text, num, mousePosition2, num3);
				this.HighlightStart = false;
				this.SetCursor(this.SelectedTextBegin, true, false);
			}
			if (!this.HighlightEnd)
			{
				this.SelectedTextEnd = this.FindCharacterPosition(this.VisibleText, text, num, mousePosition2, num3);
		...
		public int FindNextWordPosition(int direction)
		{
			...
			int result = 0;
			int result2 = this.VisibleText.Length;
		...
		private int FindCharacterPosition(string fullText, string text, float scale, Vector2 mousePosition, int omitCount)
		{
			...
			while (i < length)
			{
				int result = i + omitCount;
				float num = fontData.Font.GetWordWidth(text.Substring(0, i + 1), 0f) * scale;
				if (num > mousePosition.X)
				{
					return result;
				}
				i++;
