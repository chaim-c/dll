	...
	public class PreloadHelper
	{
		...
		public void WaitForMeshesToBeLoaded()
		{
			...
			do
			{
				...
				{
					num += ((PhysicsShape.GetFromResource(bodyName, true) == null) ? 1 : 0);
				}
				Thread.Sleep(1);
			}
			while (num != 0);
		...
		private void PreloadMeshesAndPhysics()
		{
			...
			foreach (string bodyName in this._uniqueDynamicPhysicsShapeName)
			{
				PhysicsShape.AddPreloadQueueWithName(bodyName, new Vec3(1f, 1f, 1f, -1f));
			}
		...
		{
			ValueTuple<string, bool, bool> item = new ValueTuple<string, bool, bool>(metaMeshName, useTableau, useTeamColor);
			if (!string.IsNullOrEmpty(metaMeshName) && !this._uniqueMetaMeshNames.Contains(item))
			{
				this.RegisterMetaMeshUsageIfValid(MetaMesh.GetCopy(metaMeshName, false, true), useTableau, useTeamColor);
				this._uniqueMetaMeshNames.Add(item);
			}
		}
		...
		private void RegisterMetaMeshUsageIfValid(MetaMesh metaMesh, bool useTableau, bool useTeamColor)
		{
			if (metaMesh != null)
			{
				ValueTuple<MetaMesh, bool, bool> item = new ValueTuple<MetaMesh, bool, bool>(metaMesh, useTableau, useTeamColor);
				this._uniqueMetaMeshes.Add(item);
			}
		}
