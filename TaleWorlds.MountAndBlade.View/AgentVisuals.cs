...
namespace TaleWorlds.MountAndBlade.View
{
	...
	public class AgentVisuals : IAgentVisual
	{
		...
		public float GetAnimationParameterAtChannel(int channelIndex)
		{
			...
			{
				result = this._data.AgentVisuals.GetSkeleton().GetAnimationParameterAtChannel(channelIndex);
			}
			return result;
		}
		...
		public void Refresh(bool needBatchedVersionForWeaponMeshes, AgentVisualsData data, bool forceUseFaceCache = false)
		{
			...
			this._data = data;
			bool removeSkeleton = data2.SkeletonTypeData != this._data.SkeletonTypeData;
			Equipment equipmentData = this._data.EquipmentData;
			this.Refresh(needBatchedVersionForWeaponMeshes, removeSkeleton, equipmentData, false, forceUseFaceCache);
		}
		...
		private void Refresh(bool needBatchedVersionForWeaponMeshes, bool removeSkeleton = false, Equipment oldEquipment = null, bool isRandomProgress = false, bool forceUseFaceCache = false)
		{
			...
			if (skeleton != null && this._data.ActionSetData.IsValid)
			{
				...
				if (flag)
				{
					...
					a = MBSkeletonExtensions.GetSkeletonFaceAnimationName(skeleton);
				}
			}
			else
			{
			...
			if (obj2 == null || !flag2)
			{
				...
				if (this._data.ActionSetData.IsValid && a != "facegen_teeth")
				{
				...
				{
					int mask = 481;
					this.AddSkinMeshesToEntity(mask, !needBatchedVersionForWeaponMeshes, forceUseFaceCache);
				}
				else if (!string.IsNullOrEmpty(this._data.MountCreationKeyData) || !flag)
				{
					MountVisualCreator.AddMountMeshToEntity(this.GetEntity(), this._data.EquipmentData[EquipmentIndex.ArmorItemEndSlot].Item, this._data.EquipmentData[EquipmentIndex.HorseHarness].Item, this._data.MountCreationKeyData, null);
				}
				else
				{
			...
			if (this._data.ActionSetData.IsValid && actionIndexCache != ActionIndexCache.act_none)
			{
				...
				if (skeleton != null)
				{
					skeleton.SetAgentActionChannel(0, actionIndexCache, channelParameter, blendPeriodOverride, true);
					if (num > 0f)
					{
		...
		private void AddSkinMeshesToEntity(int mask, bool useGPUMorph, bool forceUseFaceCache = false)
		{
			...
			if (this._data.EquipmentData != null)
			{
				...
				skinParams = new SkinGenerationParams(mask, this._data.EquipmentData.GetUnderwearType(isFemale), (int)this._data.EquipmentData.BodyMeshType, (int)this._data.EquipmentData.HairCoverType, (int)this._data.EquipmentData.BeardCoverType, (int)this._data.EquipmentData.BodyDeformType, this._data.PrepareImmediatelyData, 0f, (int)this._data.SkeletonTypeData, this._data.RaceData, this._data.UseTranslucencyData, this._data.UseTesselationData);
			}
			else
			{
				skinParams = new SkinGenerationParams(mask, Equipment.UnderwearTypes.FullUnderwear, 0, 4, 0, 0, this._data.PrepareImmediatelyData, 0f, (int)this._data.SkeletonTypeData, this._data.RaceData, this._data.UseTranslucencyData, this._data.UseTesselationData);
			}
		...
		public void MakeRandomVoiceForFacegen()
		{
			...
			entity.Skeleton.SetAgentActionChannel(1, AgentVisuals.act_command_leftstance_cached, 0f, -0.2f, true);
			...
			this._data.AgentVisuals.MakeVoice(index, position);
		}
			...
			if (flag)
			{
				...
				while (k < 5)
				{
					if (!this._data.EquipmentData[k].IsEmpty)
					{
						...
						this._data.AgentVisuals.AddWeaponToAgentEntity(k, weaponData, missionWeapon.GetWeaponStatsData(), weaponData2, ammoWeaponStatsData, null);
					}
					k++;
					num2++;
				}
