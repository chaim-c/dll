...
namespace TaleWorlds.MountAndBlade.View
{
	...
	public class BannerVisual : IBannerVisual
	{
		...
		public static MatrixFrame GetMeshMatrix(ref Mesh mesh, float marginLeft, float marginTop, float width, float height, bool mirrored, float rotation, float deltaZ)
		{
			...
			identity.rotation.ApplyScaleLocal(new Vec3(x, y, 1f, -1f));
		...
		public MetaMesh ConvertToMultiMesh()
		{
			...
			MatrixFrame meshMatrix = BannerVisual.GetMeshMatrix(ref mesh, bannerData.Position.x, bannerData.Position.y, bannerData.Size.x, bannerData.Size.y, bannerData.Mirror, bannerData.RotationValue * 2f * 3.1415927f, 0.5f);
			...
			for (int i = 1; i < this.Banner.BannerDataList.Count; i++)
			{
				...
				if (fromResource2 != null)
				{
					...
					int patchNode = mesh2.AddFaceCorner(new Vec3(0.5f, -0.5f, 0f, -1f), new Vec3(0f, 0f, 1f, -1f), uvCoord, uint.MaxValue, uintPtr);
					int num4 = mesh2.AddFaceCorner(new Vec3(0.5f, 0.5f, 0f, -1f), new Vec3(0f, 0f, 1f, -1f), vec + new Vec2(0.25f, 0f), uint.MaxValue, uintPtr);
					int patchNode2 = mesh2.AddFaceCorner(new Vec3(-0.5f, 0.5f, 0f, -1f), new Vec3(0f, 0f, 1f, -1f), vec, uint.MaxValue, uintPtr);
					...
					meshMatrix = BannerVisual.GetMeshMatrix(ref mesh2, bannerData2.Position.x, bannerData2.Position.y, bannerData2.Size.x, bannerData2.Size.y, bannerData2.Mirror, bannerData2.RotationValue * 2f * 3.1415927f, (float)i);
