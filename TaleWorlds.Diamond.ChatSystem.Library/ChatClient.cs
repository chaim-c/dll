	...
	public class ChatClient
	{
		...
		public async void Connect()
		{
			try
			{
				...
				string str = Base64Helper.Base64UrlEncode(AesHelper.Encrypt(Common.SerializeObject(new UserInfo(this._userName, this._displayName)), Parameters.Key, Parameters.InitializationVector));
				await this._socket.ConnectAsync(new Uri(this._endpoint + "?user=" + str), CancellationToken.None);
		...
		public void OnTick()
		{
			...
			ChatMessage message;
			if (this._receivedMessages.TryDequeue(out message))
			{
		...
		private async Task Receive()
		{
			...
			for (;;)
			{
				using (MemoryStream ms = new MemoryStream())
				{
					...
					{
						foreach (ChatMessage item in JsonConvert.DeserializeObject<ChatMessage[]>(await reader.ReadToEndAsync()))
						{
							this._receivedMessages.Enqueue(item);
						}
					}
