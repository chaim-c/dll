...
namespace TaleWorlds.Localization
{
	...
	internal class LanguageData
	{
		...
		private void Deserialize(XmlNode node, string modulePath)
		{
			...
			if (text3 != null)
			{
				string[] second = text3.Split(new char[]
				{
					','
				});
				this.SupportedIsoCodes = new List<string>(this.SupportedIsoCodes).Union(second).ToArray<string>();
			}
			XmlAttribute xmlAttribute4 = node.Attributes["text_processor"];
			string textProcessor = (xmlAttribute4 != null) ? xmlAttribute4.Value : null;
			if (!string.IsNullOrEmpty(text))
			{
				this.TextProcessor = textProcessor;
			}
			...
			if (xmlAttribute5 != null)
			{
				...
				bool.TryParse(xmlAttribute5.Value, out isUnderDevelopment);
				this.IsUnderDevelopment = isUnderDevelopment;
			}
			this.IsValid = (this.SupportedIsoCodes.Length != 0);
			if (node.HasChildNodes)
			{
				for (node = node.FirstChild; node != null; node = node.NextSibling)
				{
					if (node.Name == "LanguageFile" && node.NodeType != XmlNodeType.Comment && node.Attributes != null)
					{
						...
						string text4 = (xmlAttribute6 != null) ? xmlAttribute6.Value : null;
						if (!string.IsNullOrEmpty(text4) && !this._xmlPaths.Contains(text4))
						{
							this._xmlPaths.Add(modulePath + "/" + text4);
						}
					}
					if (node.Name == "VoiceFile" && node.NodeType != XmlNodeType.Comment && node.Attributes != null)
					{
						...
						string text5 = (xmlAttribute7 != null) ? xmlAttribute7.Value : null;
						string key = modulePath + "/" + text5;
						if (!string.IsNullOrEmpty(text5) && !this._voiceXmlPathsAndModulePaths.ContainsKey(key))
						{
							this._voiceXmlPathsAndModulePaths.Add(key, modulePath);
						}
					}
				}
			}
