...
namespace TaleWorlds.ServiceDiscovery.Client
{
	...
	public class RemoteDiscoveryService : IDiscoveryService
	{
		...
		async Task<ServiceAddress[]> IDiscoveryService.ResolveService(string service, string tag)
		{
			...
			try
			{
				result = JsonConvert.DeserializeObject<ServiceAddress[]>(await HttpHelper.DownloadStringTaskAsync(string.Concat(new string[]
				{
					this._address,
					"Data/Resolve/",
					service,
		...
		async Task<ServiceAddress[]> IDiscoveryService.DiscoverServices()
		{
			...
			try
			{
				using (HttpClient client = new HttpClient())
				{
					...
					result = JsonConvert.DeserializeObject<ServiceAddress[]>(await httpResponseMessage.Content.ReadAsStringAsync());
				}
