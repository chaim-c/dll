...
namespace psai.net
{
	...
	internal class Logik
	{
		...
		static Logik()
		{
			Logik.m_stopWatch = new Stopwatch();
			Logik.s_random = new Random();
		...
		private Logik()
		{
			...
			for (int i = 0; i < 9; i++)
			{
				...
				this.m_playbackChannels[i] = new PlaybackChannel(this, isMainChannel);
			}
		...
		private Logik(string pathToPcbFile) : this()
		{
		...
		internal PsaiResult LoadSoundtrackFromProjectFile(string pathToProjectFile)
		{
			...
			using (Stream streamOnPsaiSoundtrackFile = this.m_platformLayer.GetStreamOnPsaiSoundtrackFile(this.m_psaiCoreSoundtackFilepath))
			{
				...
				{
					result = PsaiResult.file_notFound;
				}
				else
				{
					...
					if (psaiProject != null)
					{
						result = this.LoadSoundtrackByPsaiProject(psaiProject, pathToProjectFile);
					}
					else
					{
						result = PsaiResult.error_file;
					}
				}
			}
			return result;
		...
		internal PsaiResult AddToCurrentIntensity(float deltaIntensity, bool resetIntensityFalloffToFullMusicDuration)
		{
			if (this.m_psaiState == PsaiState.playing && this.m_psaiPlayMode == PsaiPlayMode.regular)
			{
				...
				{
					float intensity = Logik.ClampPercentValue(this.getCurrentIntensity() + deltaIntensity);
					this.SetCurrentIntensityAndMusicDuration(intensity, this.m_lastMusicDuration, resetIntensityFalloffToFullMusicDuration);
				}
				return PsaiResult.OK;
			}
			...
			if (psaiStateIntended != PsaiState.silence)
			{
				if (psaiStateIntended != PsaiState.rest)
				{
					...
					else if (this.m_psaiPlayMode == PsaiPlayMode.regular && this.m_latestEndOfSegmentTriggerCall.themeId != -1)
					{
						...
						if (this.m_currentSegmentPlaying.MapOfNextTransitionSegmentToTheme.TryGetValue(themeById.id, out targetSnippet))
						{
							this.PlaySegment(targetSnippet, false);
							return;
						}
		...
		private void PushEffectiveThemeToThemeQueue(PsaiPlayMode playModeToReturnTo)
		{
			if (this.m_psaiState == PsaiState.rest)
			{
				int restTimeMillis = Logik.GetTimestampMillisElapsedSinceInitialisation() - this.m_timeStampRestStart;
				this.m_timerWakeUpFromRest.Stop();
				this.pushThemeToThemeQueue(this.m_lastBasicMood.id, this.m_lastBasicMood.intensityAfterRest, 0, true, restTimeMillis, PsaiPlayMode.regular, false);
				return;
			}
			if (this.m_latestEndOfSegmentTriggerCall.themeId != -1)
			{
		...
		internal PsaiInfo getPsaiInfo()
		{
			...
			int lastBasicMoodThemeId = (lastBasicMood != null) ? lastBasicMood.id : -1;
			...
			return new PsaiInfo(psaiState, psaiStateIntended, lastBasicMoodThemeId, effectiveThemeId, upcomingThemeId, currentIntensity, upcomingIntensity, count, (targetSegment != null) ? targetSegment.Id : -1, this.m_holdIntensity, this.m_returnToLastBasicMoodFlag, this.m_timerWakeUpFromRest.IsSet() ? this.m_timerWakeUpFromRest.GetRemainingMillisToFireTime() : 0, this.m_paused);
		}
