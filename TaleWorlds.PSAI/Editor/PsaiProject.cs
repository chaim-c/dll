	...
	public class PsaiProject : ICloneable
	{
		...
		public static PsaiProject LoadProjectFromXmlFile(string filename)
		{
			...
			try
			{
				...
				if (fileStream != null)
				{
					result = PsaiProject.LoadProjectFromStream(fileStream);
				}
				else
				{
					result = null;
		...
		public bool ConvertProjectFile_From_Legacy_To_0_9_12(string pathToProjectFile)
		{
			...
			{
				Stream input = new FileStream(pathToProjectFile, FileMode.Open, FileAccess.Read);
		...
		public void ReconstructReferencesAfterXmlDeserialization()
		{
			...
			foreach (Theme theme2 in this._themes)
			{
				...
				foreach (int themeId in theme2.Serialization_ManuallyBlockedThemeIds)
				{
				...
				foreach (Group group2 in theme2.Groups)
				{
					...
					foreach (int id in group2.Serialization_ManuallyBlockedGroupIds)
					{
						Group groupBySerializationId = this.GetGroupBySerializationId(id);
					...
					foreach (int id2 in group2.Serialization_ManuallyLinkedGroupIds)
					{
						Group groupBySerializationId2 = this.GetGroupBySerializationId(id2);
					...
					foreach (int id3 in group2.Serialization_ManualBridgeSegmentIds)
					{
						Segment snippetById = this.GetSnippetById(id3);
						if (snippetById != null)
						{
					...
					foreach (Segment segment2 in group2.Segments)
					{
						...
						foreach (int id4 in segment2.Serialization_ManuallyBlockedSegmentIds)
						{
							Segment snippetById2 = this.GetSnippetById(id4);
						...
						foreach (int id5 in segment2.Serialization_ManuallyLinkedSegmentIds)
						{
							Segment snippetById3 = this.GetSnippetById(id5);
		...
		public HashSet<Group> GetGroupsOfAllThemes()
		{
			...
			foreach (Theme theme in this.Themes)
			{
				foreach (Group item in theme.Groups)
				{
					hashSet.Add(item);
				}
			}
		...
		public object Clone()
		{
			...
			foreach (Theme theme in this.Themes)
			{
				Theme entity = (Theme)theme.Clone();
			...
			foreach (Theme theme2 in dictionary.Keys)
			{
				Theme theme3 = dictionary[theme2];
				foreach (Theme theme4 in theme2.ManuallyBlockedTargetThemes)
				{
					if (dictionary.Keys.Contains(theme4))
					{
						theme3.ManuallyBlockedTargetThemes.Add(dictionary[theme4]);
					}
				}
				for (int i = 0; i < theme2.Groups.Count; i++)
				{
					Group key = theme2.Groups[i];
					Group value = theme3.Groups[i];
					dictionary3.Add(key, value);
				}
			}
			foreach (Group group in dictionary3.Keys)
			{
				foreach (Group group2 in group.ManuallyLinkedGroups)
				{
					if (dictionary3.Keys.Contains(group2))
					{
						dictionary3[group].ManuallyLinkedGroups.Add(dictionary3[group2]);
					}
				}
				foreach (Group group3 in group.ManuallyBlockedGroups)
				{
					if (dictionary3.Keys.Contains(group3))
					{
						dictionary3[group].ManuallyBlockedGroups.Add(dictionary3[group3]);
					}
				}
				foreach (Segment segment in group.ManualBridgeSnippetsOfTargetGroups)
				{
					if (dictionary2.Keys.Contains(segment))
					{
						Segment item = dictionary2[segment];
						dictionary3[group].ManualBridgeSnippetsOfTargetGroups.Add(item);
					}
				}
			}
			foreach (Segment segment2 in segmentsOfAllThemes)
			{
				foreach (Segment segment3 in segment2.ManuallyBlockedSnippets)
				{
					if (dictionary2.Keys.Contains(segment3))
					{
						dictionary2[segment2].ManuallyBlockedSnippets.Add(dictionary2[segment3]);
					}
				}
				foreach (Segment segment4 in segment2.ManuallyLinkedSnippets)
				{
					...
					{
						dictionary2[segment2].ManuallyLinkedSnippets.Add(dictionary2[segment4]);
					}
				}
			}
			return psaiProject;
