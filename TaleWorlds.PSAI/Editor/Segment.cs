...
namespace psai.Editor
{
	...
	public class Segment : PsaiMusicEntity, ICloneable
	{
		...
		public override object Clone()
		{
			...
			foreach (Segment item in this._manuallyBlockedSnippets)
			{
				segment._manuallyBlockedSnippets.Add(item);
			}
			foreach (Segment item2 in this._manuallyLinkedSnippets)
			{
				segment._manuallyLinkedSnippets.Add(item2);
			}
			return segment;
		}
		...
		public void BuildCompatibleSegmentsSet(PsaiProject project)
		{
			...
			foreach (Segment segment in segmentsOfAllThemes)
			{
				...
				if (flag)
				{
					...
					this.AddCompatibleSnippet(segment, compatibility);
				}
			}
		}
		...
		public int CreateSegmentSuitabilityBitfield(PsaiProject parentProject)
		{
			...
			{
				Segment.SetSegmentSuitabilityFlag(ref result, SegmentSuitability.bridge);
			}
			if (this.IsUsableAtStart)
			{
				Segment.SetSegmentSuitabilityFlag(ref result, SegmentSuitability.start);
			}
			if (this.IsUsableInMiddle)
			{
				Segment.SetSegmentSuitabilityFlag(ref result, SegmentSuitability.middle);
			}
			if (this.IsUsableAtEnd)
			{
				Segment.SetSegmentSuitabilityFlag(ref result, SegmentSuitability.end);
			}
			return result;
		}
