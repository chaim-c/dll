	...
	public class AudioData : ICloneable
	{
		...
		public string FilePathRelativeToProjectDir
		{
			...
			set
			{
				...
				this._filePathRelativeToProjectDir = filePathRelativeToProjectDir;
			}
		}
			...
			if (fullPathToAudioFile != null && fullPathToAudioFile.Length > 0)
			{
				...
				if (File.Exists(text))
				{
					...
					if (stream != null)
					{
						...
						int sampleRate;
						int bitsPerSample;
						int lengthOfWaveformDataInBytes;
						long byteIndexOfWaveformDataWithinAudioFile;
						if (AudioData.ReadWaveHeader(stream, out channelCount, out sampleRate, out bitsPerSample, out lengthOfWaveformDataInBytes, out byteIndexOfWaveformDataWithinAudioFile) == PsaiResult.OK)
						{
							this.ChannelCount = channelCount;
							this.SampleRate = sampleRate;
							this.LengthOfWaveformDataInBytes = lengthOfWaveformDataInBytes;
							this.BitsPerSample = bitsPerSample;
							this.ByteIndexOfWaveformDataWithinAudioFile = byteIndexOfWaveformDataWithinAudioFile;
							this.TotalLengthInSamples = AudioData.CalculateTotalLengthInSamples(lengthOfWaveformDataInBytes, bitsPerSample, channelCount);
							errorMessage = "";
							result = true;
						}
						else
		...
		public static PsaiResult ReadWaveHeader(Stream stream, out int outChannelCount, out int outSampleRate, out int outBitsPerSample, out int outLengthOfWaveformDatablockInBytes, out long outBytePositionOfWaveformData)
		{
			...
			long offset = position + (long)num + 4L;
