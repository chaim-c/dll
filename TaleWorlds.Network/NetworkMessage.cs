...
namespace TaleWorlds.Network
{
	...
	public class NetworkMessage : INetworkMessageWriter, INetworkMessageReader
	{
		...
		{
			int result = BitConverter.ToInt32(this.Buffer, this._readCursor);
			this._readCursor += 4;
			return result;
		}
		...
		{
			short result = BitConverter.ToInt16(this.Buffer, this._readCursor);
			this._readCursor += 2;
			return result;
		}
		...
		{
			bool result = BitConverter.ToBoolean(this.Buffer, this._readCursor);
			this._readCursor++;
			return result;
		}
		...
		{
			byte result = this.Buffer[this._readCursor];
			this._readCursor++;
			return result;
		}
		...
		public string ReadString()
		{
			...
			if (num >= 0)
			{
				result = Encoding.UTF8.GetString(this.Buffer, this._readCursor, num);
		...
		{
			float result = BitConverter.ToSingle(this.Buffer, this._readCursor);
			this._readCursor += 4;
			return result;
		}
		...
		{
			long result = BitConverter.ToInt64(this.Buffer, this._readCursor);
			this._readCursor += 8;
			return result;
		}
		...
		{
			ulong result = BitConverter.ToUInt64(this.Buffer, this._readCursor);
			this._readCursor += 8;
			return result;
		}
