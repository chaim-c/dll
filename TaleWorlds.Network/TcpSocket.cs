	...
	internal class TcpSocket
	{
		...
		internal void Connect(string address, int port)
		{
			try
			{
				...
				this._socketAsyncEventArgsListener.RemoteEndPoint = new IPEndPoint(address2, port);
			...
			catch (Exception arg)
			{
				...
				Debug.Print("Tcp Connection Error: " + arg, 0, Debug.DebugColor.White, 17592186044416UL);
				Thread.Sleep(250);
			}
		}
		...
		internal void ProcessWrite()
		{
			if (this.IsConnected && this._currentlySendingMessage == null && this._writeNetworkMessageQueue.TryDequeue(out this._currentlySendingMessage))
			{
				...
				{
					Debug.Print("SendMessage Error: " + arg, 0, Debug.DebugColor.White, 17592186044416UL);
					this.Status = TcpStatus.ConnectionClosed;
				}
			}
		}
		...
		private void ProcessIO(object sender, SocketAsyncEventArgs e)
		{
			...
			catch (Exception ex)
			{
				...
				{
					"Exception on TcpSocket::ProcessIO ",
					ex,
					" - ",
					ex.Message,
					" ",
